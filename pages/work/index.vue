<template>
  <nuxt-layout>
    ...
    <ul class="list-style:none m:0 p:0 grid-cols:2 gap:30">
      <li v-for="{ _path: slug, title, excerpt, image } in work" :key="slug" class="r:5 b:1|solid|#ccc bg:#fff">
        <nuxt-link :to="slug" class="text-decoration:none color:#222">
          <nuxt-img v-if="image" :src="image" width="500" height="300" fit="cover" format="webp" loading="lazy" :alt="title" class="bb:1|solid|#ccc" />
